...
for($i = 0; $i < count($options); $i++)
{
	...
	{
		if($options[$i]->id === $secondOptions[$j]->id)
		{
			continue;
		}
				
		$probability = pOfThingAndThing($options[$i], $secondOptions[$j], $combos);
	}
...
function pOfThing(&$thing)
{
	...
	$q  = "SELECT tool_has_option.id, tool_has_option.decision_id, decision.name FROM `tool_has_option` INNER JOIN decision ON decision.id = tool_has_option.decision_id WHERE option_id = $thing->id";
	$retval = $database->query($q);
	if(!$retval)
	{
		echo "Fail: $q<br />";
