...
else if($_POST["what"] === "thing split")
{
	$q = "INSERT INTO `split_is_thing` VALUES('', $_POST[custom], $_POST[option], 0)";
	$ret = $db->q($q);
	if(!$ret)
	{
		echo "Fail: $q<br />";
...
else if($_POST["what"] === "done new")
{
	...
	else
	{
		...
		$q = "INSERT INTO `progress_check` VALUES('', $_POST[custom], 0, 0, 0, 1, 0, 0, 0)";
		$ret = $db->q($q);
		if(!$ret)
		{
			echo "Fail: $q<br />";
...
else if($_POST["what"] === "done option")
{
	...
	{
		// If there's no, insert one
		$q = "INSERT INTO `progress_check` VALUES('', $_POST[custom], 1, 0, 0, 0, 0, 0)";
		$ret = $db->q($q);
		if(!$ret)
		{
			echo "Fail: $q<br />";
...
else if($_POST["what"] === "done decision")
{
	...
	{
		// If there's no, insert one
		$q = "INSERT INTO `progress_check` VALUES('', $_POST[custom], 1, 1, 0, 0, 0, 0)";
		$ret = $db->q($q);
		if(!$ret)
		{
			echo "Fail: $q<br />";
