<html>
	<head>
		...
		<?php
			...
			$q = "SELECT * FROM `thing` WHERE type = 'category' AND stage_removed = -1";
			$ret = $db->q($q);
			if(!$ret)
			{
				echo "Fail: $q<br />";
...
foreach($rows AS $row)
{
	...
	while($r = mysqli_fetch_array($ret))
	{
		$q2 = "SELECT * FROM `split_is_option` WHERE split_id = $r[id]";
		$ret2 = $db->q($q2);
		if(!$ret2)
		{
			echo "Fail: $q2<br />";
		...
		$q2 = "SELECT * FROM `split_is_decision` WHERE split_id = $r[id]";
		$ret2 = $db->q($q2);
		if(!$ret2)
		{
			echo "Fail: $q2<br />";
		...
		$r2 = mysqli_fetch_array($ret2);
		if($r2["done_ds"] == 1) // If the entry is 1 it's been done, move on.
			continue;
