<html>
	<head>
		...
		<?php
			...
			{
				$options .= "{id:'$r[id]',name:\"" . trim(addslashes($r["name"])) . "\",type:'$r[type]'},\n";
			}
		...
		function filter(event)
		{
			...
			{
				var colour = results[i].type == "option" ? "#000" : results[i].type == "decision" ? "#000080" : "#54A854";
				var result = $("<li class='bob' thing_id='" + results[i].id + "' custom_id='" + id + "' onclick='choose(this)' style='color:" + colour + "'>" + results[i].name + "</li>");
...
foreach($rows AS $row)
{
	...
	while($r = mysqli_fetch_array($ret))
	{
		...
		if($r2["done_option"] == 1  && $r2["done_verify"] == 1) // If the entry is 1 it's been done, move on.
			continue;
...
for($i = 0; $i < count($customs); $i++)
{
	...
	echo "<div id='contents$r[id]'>
		...
		<button id='notoption$r[id]' type='button' onclick='notOption(this)' style='margin-right:20px;'>Not option</button><br />
