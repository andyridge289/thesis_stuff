		...
		function getForIndex(customID)
		{
			$.ajax({
				...
				for(var i = 0; i < data.unverified.length; i++)
				{
					...
					var div = $("<span id='thing" + id + "_" + thing.id + "' style='margin-left:20px;color:" + textColor + " '>" + thing.name + "</span>");
					

					thingBox.append(okButton);
					thingBox.append(notOkButton);
					thingBox.append(div);
					thingBox.append($("<br />"));
				}
...
$matrix = array();
$q = "SELECT * FROM `split_is_thing`";
$ret = $db->q($q);
if(!$ret){ echo "Fail: $q<br />"; return; }
while($r = mysqli_fetch_array($ret))
{
	if(!array_key_exists($r["split_id"], $matrix))
	{
		$matrix[$r["split_id"]] = array($r["thing_id"]);
	}
	else
	{
		array_push($matrix[$r["split_id"]], $r["thing_id"]);
	}
}
...
$q = "SELECT pcs.split_id AS id, sc.name AS name, sc.description AS description, sc.rationale AS rationale
	...
	WHERE pc.split = 3 AND pc.ignore_custom = 0 AND pcs.done_verify = 0";
...
// $q = "SELECT ct.split_id AS custom_id
// 	FROM  `split_is_thing` AS ct
// 	LEFT JOIN  `thing` AS t ON ct.thing_id = t.id
// 	LEFT JOIN `participant_has_custom` AS pc on ct.split_id = pc.id
...
// 	$q1 = "UPDATE progress_check_split SET done_verify = 0, done_option = 0 WHERE split_id = $r[custom_id]";
// 	$ret1 = $db->q($q1);
// 	if(!$ret1)
// 	{
// 		echo "Fail $q1<br />";
