...
echo "var data = { \"custom_id\": $_POST[split],
	\"verified\":";

// Need to get the things that are linked to the custom that ARE verified
// $q = "SELECT ct.id AS ct_id, ct.thing_id, t.name AS thing_name, ct.verified AS verified, t.type AS type
// 		FROM `custom_is_thing` AS ct
// 		LEFT JOIN `thing` AS t on ct.thing_id = t.id
// 		WHERE ct.custom_id = $_POST[split]";
$q = "SELECT st.id AS ct_id, st.thing_id, t.name AS thing_name, st.verified AS verified, 
	t.type AS type, t.stage_removed AS stage_removed
	FROM `split_is_thing` AS st
	LEFT JOIN `thing` AS t ON st.thing_id = t.id
	WHERE st.split_id = $_POST[split]";
$ret = $db->q($q);
if(!$ret)
{
	echo "Fail: $q<br />";
...
$q = "SELECT name, description, rationale FROM `split_custom` WHERE id = $_POST[split]";
$ret = $db->q($q);
if(!$ret)
{
	echo "Fail: $q<br />";
...
{
	$q = "SELECT * FROM split_is_thing WHERE thing_id = $match->id AND split_id = $_POST[split]";
	$ret = $db->q($q);
	if(!$ret)
	{
		echo "Fail: $q<br />";
