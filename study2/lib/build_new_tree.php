...
function setInfo(&$node)
{
	...
	if(strcmp($node->name, "") == 0)
	{
		...
		$node->name = $row["name"];
		$node->type = $row["type"];
		$node->step = $row["stage_added"];
		$node->description = $row["description"];
	}

	for($i = 0; $i < count($node->children); $i++)
	{
		if($node->children[$i]->parent == null)
			$node->children[$i]->parent = $node;

		setInfo($node->children[$i]);

		//echo $node->children[$i]->name . ": " . $node->name . "<br />";
	}
}
...
function reverseString($node)
{
	...
	$oldNode = findInTree($newRoot, $node->name);
	$temp = $oldNode;
	$tree = "";

	while($temp != null && $temp->parent->parent != null)
	{
		$tree = "<span class='label'>" . $temp->parent->name . "</span>" . " &gt; " . $tree;
		$temp = $temp->parent;
	}	
